...
public class Chapter04 {
	...
	public void testListItem(Jedis conn, boolean nested) {
		...
		System.out
				.println("We need to set up just enough state so that a user can list an item");
		String seller = "userX";
		String item = "itemX";
	...
	public void testPurchaseItem(Jedis conn) {
		...
		System.out
				.println("We need to set up just enough state so a user can buy an item");
	...
	public boolean listItem(Jedis conn, String itemId, String sellerId,
			double price) {
	...
	public void benchmarkUpdateToken(Jedis conn, int duration) {
		try {
			...
			Class[] args = new Class[] { Jedis.class, String.class,
					String.class, String.class };
			Method[] methods = new Method[] {
					this.getClass().getDeclaredMethod("updateToken", args),
					this.getClass().getDeclaredMethod("updateTokenPipeline",
							args), };
	...
	public void updateTokenPipeline(Jedis conn, String token, String user,
			String item) {
		long timestamp = System.currentTimeMillis() / 1000;
