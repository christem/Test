...
public class RedisCluster {
	...
	for (int i = 0; i < 10; i++) {
	    ...
	    String keys = "clusterTest" + i;
	    // 这东西 可以直接看到key 的分片数，就能知道放哪个 节点
	    System.out.println(keys + "-->" + JedisClusterCRC16.getSlot(keys));

	    jc.set(keys, "Redis cluster" + i);
	    // 获取存储的数据并输出
	    System.out.println("string in redis:: " + jc.get(keys));
	}
    }
}
